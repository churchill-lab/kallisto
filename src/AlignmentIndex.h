...
#ifndef KALLISTO_ALIGNMENT_H
#define KALLISTO_ALIGNMENT_H
...
void createAlignments(Index& index, const ProgramOptions& opt, TranscriptCollector& tc) {
    ...
    if (paired) {
        std::cerr << "[align] Mode: paired-end" << std::endl;
    } else {
        std::cerr << "[align] Mode: single-end" << std::endl;
    }

    for (int i = 0; i < opt.files.size(); i += (paired) ? 2 : 1) {
        if (paired) {
            std::cerr << "[align] Processing pair " << (i / 2 + 1) << ": " << opt.files[i] << std::endl
            << "                        " << opt.files[i + 1] << std::endl;
        } else {
            std::cerr << "[align] Processing " << i + 1 << ": " << opt.files[i] << std::endl;
        }
    }

    // for each file
    std::cerr << "[align] Aligning..." << std::endl;
    std::cerr.flush();
    ...
    std::cerr << "[align] Done" << std::endl << std::endl;

    std::cerr << "[align]    Haplotypes: " << haps.size() << std::endl;
    std::cerr << "[align] Total Targets: " << idx << std::endl;
    std::cerr << "[align]  Main Targets: " << main_targets.size() << std::endl;
    std::cerr << "[align]         Reads: " << numreads << std::endl;
    std::cerr << "[align]  Mapped Reads: " << nummapped << std::endl;
    std::cerr << "[align]    Alignments: " << num_alignment_rows << std::endl;

    std::ofstream out;
    out.open(opt.alignment, std::ios::out | std::ios::binary);

    if (!out.is_open()) {
        std::cerr << "Error: align output file could not be opened!";
        exit(1);
    }
...
void loadAlignments(const ProgramOptions& opt) {
    std::string index_in = opt.alignment;
    std::ifstream in;
    ...
    if (ec_read) {
        ...
        std::cerr << "[alignread] Number of EC Classes: " << ec_classes_size << std::endl;
    } else {
        ...
        std::cerr << "[alignread] Number of reads: " << reads1_size << std::endl;
    }
    ...
    std::cerr << "[alignread] Number of Main Targets: " << main_target_size << std::endl;
    ...
    std::cerr << "[alignread] Number of Haplotypes: " << hap_size << std::endl;
    ...
    if (ec_read) {
        ...
        std::cerr << "[alignread] Number of Alignment Records: " << num_alignments << std::endl;
